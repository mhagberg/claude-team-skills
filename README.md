...
| `/onboard-writeback <slug> [--from-phase N]` | **End-to-end walkthrough to turn on Sage 100 Contractor WRITE-BACK for an already-onboarded customer** (Claude creates change orders / AP invoices / journal entries via the native API). Drives 4 phases, **one sub-skill each** — Sage least-priv user → operator provisioning → customer one-click install → record + verify (dry-run) — confirming before each and piping captured values (setup key, agent token, mesh IP) forward. Opens either the customer-facing or operator-facing HTML playbook. Runs after `/onboard-customer`. Customer side is **one PowerShell command**. |
| `/onboard-writeback-sage-user <slug>` | Phase 1: guides the customer's IT to create the least-privilege Sage **API-security-group** user (NOT Company Administrator; P1 #9) and captures the user/password + live company DB + data source for the later phases. |
| `/onboard-writeback-provision <slug> --company-db "<db>" --datasource <ds> --sage-user <user>` | Phase 2 (operator): wraps `Sage-API-Write-Back/deploy/provision-customer.sh` — creates the customer's NetBird group + auto-join setup key + a SCOPED **TCP-9447-only** policy (`MCPConnector → customer-<slug>`), mints the agent token (NetBird API token from Google Secret Manager), and prints the single PowerShell command + the IT-facing HTML quickstart to send the customer. |
| `/onboard-writeback-install <slug>` | Phase 3: hands over the single one-click install command + opens **either** the customer-facing (`it-writeback-quickstart.html`) or operator-facing (`onboarding-playbook.html`) walkthrough, runs the installer (`install-writeback.ps1`), and captures the box's NetBird mesh IP from its self-test. |
